...
char mnemonic[fctnum][5];   /* 目标代码指令名称 */
...
int main()
{
	...
	{
		printf("table输出文件打开失败!\n");
		exit(1);
	}

	/*printf("List object codes?(Y/N)");	// 是否输出目标代码
	scanf("%s", fname);*/
	listswitch = 1;
    ...
    if (err == 0)
    {
		...
		{
			printf("fcode.txt打开失败!\n");
			exit(1);
		}

		if ((fresult = fopen("fresult.txt", "w")) == NULL)
		{
			printf("fresult.txt打开失败!\n");
			exit(1);
		}
...
void expression(bool* fsys, int* ptx, int lev)
{
	...
	if(sym == add || sym == sub)	/* 表达式开头有正负号，此时当前表达式被看作一个正的或负的项 */
	{
		addop = sym;	/* 保存开头的正负号 */
		getsym();
		memcpy(nxtlev, fsys, sizeof(bool) * symnum);
